...
def OptimizedGraphPose(curr_node_idx, graph_optimized):
...
class PoseGraphResultSaver:
    ...
    def OptimizedPoseGraphUpdate(self, cur_node_idx, graph_optimized):

        # ic("OptimizedPoseGraphUpdate")
        optimized_pose_list = OptimizedGraphPose(cur_node_idx, graph_optimized)
        self.pose_list = optimized_pose_list # update with optimized pose

    def OptimizedPoseUpdate(self,cur_pose):
        # ic("OptimizedPoseUpdate")
        self.pose_list = np.vstack((self.pose_list, np.reshape(cur_pose, (-1, 16))))
    ...
    def vizCurrentTrajectory(self, fig_idx):
        ...
        plt.clf()
        plt.plot(x, y, color='blue') # kitti camera coord for clarity
