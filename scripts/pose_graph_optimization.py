...
class IcpTest:
    ...
    def callback(self,  point_cloud_ros):
        ...
        if(self.is_first_node):
            ...
            return
        self.cur_node_idx += 1 
        result = small_gicp.align(target_points = self.previous_scan_points, source_points = self.current_scan_points, downsampling_resolution=0.3,init_T_target_source = self.icp_initial,\
                                          registration_type = 'VGICP',voxel_resolution=0.3,max_corresponding_distance = 3.0,num_threads = 30)
